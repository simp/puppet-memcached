...
class memcached (
  ...
  $manage_firewall = defined('$::use_iptables') ? { true => getvar('::use_iptables'), default => hiera('use_iptables',true) },
  $iptables_allow  = defined('$::client_nets') ? { true => getvar('::client_nets'), default => hiera('client_nets',['127.0.0.1']) },
  $max_memory      = false,
  $item_size       = false,
  $lock_memory     = false,
  $listen_ip       = '0.0.0.0',
